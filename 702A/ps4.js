...
function setupRW() {
	...
	document.getElementById('load').innerHTML='<tr>'+
			...
			'<a href="#" id="goldhen" class="button" onclick="toggle_payload(\'goldhen\'); return false" style="width:28%">GoldHen</a>&nbsp;'+
			'</td>'+
			'</tr>'+
			'<tr><td><br/></td></tr>'+
			'<tr>'+
...
function toggle_payload(pld){
	if(pld=="hen" || pld=="binloader" || pld=="goldhen"){if(confirm("HEN/GOLDHEN (or) Binloader cannot be loaded with any other payloads\nDo you still want to load this payload alone ?")){document.getElementById("pldooe").value = pld; load_payload();}}
	else{
